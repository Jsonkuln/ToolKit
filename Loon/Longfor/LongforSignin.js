...
const CONFIG = {
    ...
    RISK_TOKEN_KEY: 'longfor_risktoken',
    ACW_COOKIE: "acw_tc=276aede117516477058858009e29e85ba7429dd0c2a1b3c6f8c5a55d36958a",
    DEBUG_MODE: false,
    RETRY_COUNT: 3,
    RETRY_DELAY: 2000,
    ...
    ACTIVITY: {
        ...
        SIGN_IN_MINI_APP_NO: "11111111111686241863606037740000",
    ...
    COMMON_HEADERS: {
        ...
        'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 18_5 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 &MAIAWebKit_iOS_com.longfor.supera_1.18.2_202506052233_Default_3.2.4.8'
    }
}
...
function getToken() {
    ...
    if (isMatch(/\/udid\/c1\/\?/)) {
        ...
        try {
            const resHead = $response.headers
            // const _cooie = headers[""]
            const _riskToken = resHead["ETag"]

            const currentRiskToken = getVal(CONFIG.RISK_TOKEN_KEY)
            if (_riskToken && _riskToken !== currentRiskToken) {
                setVal(CONFIG.RISK_TOKEN_KEY, _riskToken)
                log(`risktoken获取成功: ${_riskToken}`)
            }
            
        }catch (error) {
            logError('获取risktoken失败', error)
        }
        
    }
}

async function doSignIn(_activityNo=CONFIG.ACTIVITY.SIGN_IN_NO) {
    const token = getVal(CONFIG.TOKEN_KEY)
    if (!validateToken(token)) {
    ...
    const risktoken = getVal(CONFIG.RISK_TOKEN_KEY)
    const acw_cookie = getVal(CONFIG.ACW_COOKIE)
    ...
    try {
        ...
        const options = {
            ...
            headers: headers,
            body: JSON.stringify({"activity_no": _activityNo})
        }
